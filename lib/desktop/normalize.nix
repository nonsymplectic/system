{lib}: policy: let
  ...
  defaultBrowserCommand =
    if defaultBrowserName == "chromium"
    then mkCommand "chromium" (flagsFor "chromium")
    else if defaultBrowserName == "qutebrowser"
    then mkCommand "qutebrowser" (flagsFor "qutebrowser")
    else throw "normalizeDesktopPolicy: unsupported browser: ${defaultBrowserName}";
